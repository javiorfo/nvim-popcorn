...
local function build_popup(title, footer, width, height, border)
    ...
    local top_line = string.format("%s%s%s%s%s", border.corner_left_up, side_top, title, side_top, border
        .corner_right_up)
    ...
    local bottom_line
    if #footer == 0 then
        bottom_line = string.format("%s%s%s", border.corner_left_down, string.rep(border.horizontal, top_line_length - 2),
            border.corner_right_down)
    else
        local size_bottom_length = ((width - 4 - clean_length(footer)) / 2)
        local side_bottom = string.rep(border.horizontal, size_bottom_length)
        local side_bottom2 = string.rep(border.horizontal,
            bottom_amend(title, footer) and size_bottom_length - 1 or size_bottom_length)
        bottom_line = string.format("%s%s %s %s%s", border.corner_left_down, side_bottom, footer, side_bottom2,
            border.corner_right_down)
        if clean_length(bottom_line) < top_line_length then
            ...
            bottom_line = string.format("%s%s %s %s%s", border.corner_left_down, side_bottom, footer,
                side_bottom2 .. add_bottom, border.corner_right_down)
        end
    end
...
function popcorn:pop()
    ...
    vim.api.nvim_buf_set_keymap(0, 'n', self.callback_keymap or '<CR>',
        '<cmd>lua require("popcorn").execute_callback()<cr>', map_opts)
